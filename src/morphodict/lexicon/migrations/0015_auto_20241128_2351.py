...
from morphodict.search.wordnet import WordnetEntry
...
def migrate_semantic_domains(apps, schema_editor):
    ...
    print("Migrating semantic domain annotations (WordNet and RapidWords)...")
    for wf in tqdm(wordforms.iterator(), total=count):
        if wf.rw_indices:
